...
export function fitSlideText(slideDiv, slideImages, forceFs, forceLs) {
  ...
  const [imgEntry, imgEntry1] = getSlideImages(slideImages, slideKey);
  ...
  if (isAnswer) {
    const ansBar = slideDiv.querySelector('.answer-bar:not(.answer-bar--ghost)');
    answerH = ansBar ? ansBar.offsetHeight / PX : 0.7;
    bottomLimit = H - answerH - pad;
  }
  let twoImageFrac = 0.45;
  if (hasTwoImages) {
    const layout = computeTwoImageLayout(imgEntry.width / imgEntry.height, imgEntry1.width / imgEntry1.height, twoImageFrac);
  ...
  // Browser inline text measurement can be slightly shorter than PPTX rendering
  // due to font metric differences. Scale measured heights up to compensate.
  const HEIGHT_SAFETY = 1.05;

  // Compact layout: if text is short, move landscape image below text for more space
  // (skipped for two-image layout)
  if (!forceFs && imgEntry && !hasTwoImages && enEl) {
    const ar = imgEntry.width / imgEntry.height;
    const layout = computeImageLayout(ar);
    if (layout.mode === "landscape") {
      ...
      const deH = deEl.scrollHeight / PX * HEIGHT_SAFETY;
      const enH = enEl.scrollHeight / PX * HEIGHT_SAFETY;
      ...
      if (deH + enH <= H * 0.4) {
        ...
        const imgEl = slideDiv.querySelector(".slide-img-wrap");
        if (imgEl) {
  ...
  const ansProps = answerH != null ? { answerH } : {};
  ...
  function tryFit(fs, ls) {
    ...
    const deBottom = pad + deEl.scrollHeight / PX * HEIGHT_SAFETY;

    // DE at full width must not extend below landscape image
    if (deBottomLimit && deBottom > deBottomLimit) return null;

    if (!enEl) {
      return deBottom <= bottomLimit ? { fontSize: fs, lineSpacing: ls, enY: 2.5, ...ansProps } : null;
    }

    // 1a: push EN down if DE overlaps default position
    let enY = Math.max(2.5, deBottom + minGap);
    enEl.style.top = Math.round(enY * PX) + "px";
    if (enY + enEl.scrollHeight / PX * HEIGHT_SAFETY <= bottomLimit) {
      return { fontSize: fs, lineSpacing: ls, enY, ...ansProps };
    }
  ...
  if (forceFs != null) {
    ...
    // Doesn't fit — still apply and return best-effort enY, flag overflow
    tryFit(forceFs, forceLs);
    const enY = pad + deEl.scrollHeight / PX + pad;
    if (enEl) enEl.style.top = Math.round(enY * PX) + "px";
    return { fontSize: forceFs, lineSpacing: forceLs, enY, ...ansProps, overflow: true };
  }
  ...
  // Two images: retry with smaller image area (40%) before giving up
  if (hasTwoImages && twoImageFrac > 0.4) {
    twoImageFrac = 0.4;
    const layout = computeTwoImageLayout(imgEntry.width / imgEntry.height, imgEntry1.width / imgEntry1.height, twoImageFrac);
    bottomLimit = Math.min(isAnswer ? 4.8 : H - pad, layout.textBottomLimit);
    for (const fs of [baseFs, baseFs - 0.5, baseFs - 1, baseFs - 1.5, baseFs - 2]) {
      let lo = minLs, hi = baseLs, best = null;
      while (lo <= hi) {
        const mid = Math.round((lo + hi) / 2);
        const result = tryFit(fs, mid);
        if (result) { best = result; lo = mid + 1; } else { hi = mid - 1; }
      }
      if (best) {
        // Reposition images in DOM with reduced fraction
        const imgEls = slideDiv.querySelectorAll(".slide-img-wrap");
        if (imgEls[0]) { imgEls[0].style.left = px(layout.img0.x); imgEls[0].style.top = px(layout.img0.y); imgEls[0].style.width = px(layout.img0.w); imgEls[0].style.height = px(layout.img0.h); }
        if (imgEls[1]) { imgEls[1].style.left = px(layout.img1.x); imgEls[1].style.top = px(layout.img1.y); imgEls[1].style.width = px(layout.img1.w); imgEls[1].style.height = px(layout.img1.h); }
        return { ...best, twoImageFrac };
      }
    }
  }

  // Give up — apply smallest settings, return best effort, flag overflow
  const lastFs = Math.max(8, baseFs - 2);
  const lastResult = tryFit(lastFs, minLs);
  const enY = pad + deEl.scrollHeight / PX + pad;
  if (enEl) enEl.style.top = Math.round(enY * PX) + "px";
  return { fontSize: lastFs, lineSpacing: minLs, enY, ...ansProps, ...(hasTwoImages ? { twoImageFrac } : {}), overflow: !lastResult };
}
