...
export function layoutImageBelowText(textEl, imgEl, imgEntry) {
  ...
  layoutImageBelowY(imgEl, imgEntry, textBottom);
}

/** Convenience wrapper: dispatch to one- or two-image variant based on imgEntry1 presence. */
export function layoutImagesBelowText(textEl, imgEl, img1El, imgEntry, imgEntry1) {
  if (!imgEntry) return;
  if (imgEntry1) layoutTwoImagesBelowText(textEl, imgEl, img1El, imgEntry, imgEntry1);
  else layoutImageBelowText(textEl, imgEl, imgEntry);
}

/** Lower-level: position an image below a known textBottom Y (in inches). */
export function layoutImageBelowY(imgEl, imgEntry, textBottom) {
  if (!imgEl || !imgEntry) return;
...
export function layoutTwoImagesBelowText(textEl, img0El, img1El, entry0, entry1) {
  ...
  const textBottom = (textEl.offsetTop + textEl.offsetHeight) / PX;
  layoutTwoImagesBelowY(img0El, img1El, entry0, entry1, textBottom);
}

/** Lower-level: position two images side-by-side below a known textBottom Y (in inches). */
export function layoutTwoImagesBelowY(img0El, img1El, entry0, entry1, textBottom) {
  if (!img0El || !img1El || !entry0 || !entry1) return;
  const { pad, width: W, height: H } = SLIDE_STYLE;
  const imgTop = textBottom + pad;
  const fullW = W - 2 * pad;
  const boxH = H - pad - imgTop;
