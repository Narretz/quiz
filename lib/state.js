...
export async function downloadPptx() {
  ...
  const hasVideo = Object.values(images).some((e) => e.type === "video");
  const needsPostProcess = audioSlideNumbers.size > 0 || revealSlides.size > 0 || hasVideo;
  if (needsPostProcess) {
    const { postProcessPptx } = await import("./pptx-fix.js");
    const buf = await pptx.write({ outputType: "arraybuffer" });
    const blob = await postProcessPptx(buf, { audioDurations, audioSlideNumbers, revealSlides });
