...
/** Extract editable question map from quiz rounds. */
...
export async function loadSavedQuiz(id) {
  ...
  quizQuestions.value = saved.questions || extractQuestions(saved.quiz);
  manualOverrides.value = saved.manualOverrides || {};
  // Use stored descriptors if available, else regenerate (migration for old saves)
  if (saved.descriptors) {
    slideDescriptors.value = saved.descriptors;
  } else {
    const { buildSlideDescriptors } = await import("../quiz-core.js");
    slideDescriptors.value = buildSlideDescriptors(saved.quiz);
  }
  if (saved.style) {
    slideStyle.value = {
