...
// --- Question reordering (within a round, swap with neighbor in same phase) ---
function questionRound(desc) {
  if (!desc || desc.type !== "question") return null;
  const m = desc.id?.match(/^r(\d+)q\d+$/);
  return m ? m[1] : null;
}

export function canMoveQuestion(descIdx, direction) {
  const descs = slideDescriptors.value;
  const cur = descs[descIdx];
  const nbr = descs[descIdx + direction];
  const curRi = questionRound(cur);
  const nbrRi = questionRound(nbr);
  return curRi != null && curRi === nbrRi && cur.withAnswers === nbr.withAnswers;
}

/**
 * Swap a question descriptor with its neighbor in the same round and phase.
 * Both the current-phase pair (question or answer side) AND the opposite-phase
 * pair are swapped together so the two sides stay in lockstep. `num` is kept
 * positional — the slot's label stays put, only the identity (id) moves.
 */
export function moveQuestion(descIdx, direction) {
  if (!canMoveQuestion(descIdx, direction)) return false;
  const descs = slideDescriptors.value;
  const cur = descs[descIdx];
  const targetIdx = descIdx + direction;
  const nbr = descs[targetIdx];

  const pairOf = (d) => descs.findIndex(
    (x) => x.type === "question" && x.id === d.id && x.withAnswers !== d.withAnswers,
  );
  const curPairIdx = pairOf(cur);
  const nbrPairIdx = pairOf(nbr);

  const next = descs.slice();
  next[descIdx] = { ...nbr, num: cur.num };
  next[targetIdx] = { ...cur, num: nbr.num };
  if (curPairIdx !== -1 && nbrPairIdx !== -1) {
    const curPair = descs[curPairIdx];
    const nbrPair = descs[nbrPairIdx];
    next[curPairIdx] = { ...nbrPair, num: curPair.num };
    next[nbrPairIdx] = { ...curPair, num: nbrPair.num };
  }
  slideDescriptors.value = next;
  scheduleSave();
  return true;
}
...
function emptyQuestion() {
  return { text: { de: "", en: "" }, answers: { de: "", en: "" } };
}

function blankQuizTemplate(date) {
  return {
    date,
    rounds: [
      { name: "Round 1", description: { de: "", en: "" }, questions: Array.from({ length: 10 }, emptyQuestion) },
      { name: "Round 2", description: { de: "", en: "" }, questions: Array.from({ length: 10 }, emptyQuestion) },
      { name: "Round 3", description: { de: "", en: "" }, questions: Array.from({ length: 10 }, emptyQuestion) },
      { name: "Round 4", description: { de: "", en: "" }, questions: Array.from({ length: 10 }, emptyQuestion) },
      { name: "Name 10", description: { de: "", en: "" }, questions: [emptyQuestion()] },
      { name: "Jackpot!", description: { de: "", en: "" }, questions: Array.from({ length: 4 }, emptyQuestion) },
    ],
  };
}

export async function createBlankQuiz() {
  const { buildSlideDescriptors } = await import("../quiz-core.js");
  ...
  const quiz = blankQuizTemplate(date);
  ...
  setQuizParam(id);
  status.value = `${quiz.rounds.length} rounds`;
  scheduleSave();
}

export async function uploadQuiz(file) {
  const { astToQuiz, normalizeQuizStructure, buildSlideDescriptors } = await import("../quiz-core.js");
...
/** Basename for the downloaded .pptx: prefer user-set name > date > id. */
function pptxBasename() {
  const quiz = currentQuiz.value;
  const raw = (quiz?.name || quiz?.date || currentQuizId.value || "quiz").toString().trim();
  // Strip characters that would confuse filesystems: / \ : * ? " < > |
  return raw.replace(/[\/\\:*?"<>|]/g, "-") || "quiz";
}
...
export async function downloadPptx() {
  ...
  const fileName = `quiz-${pptxBasename()}.pptx`;
