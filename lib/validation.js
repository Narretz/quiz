...
// Multiple-choice marker at the very start of a string, e.g. "A) Foo", "b: bar", "C. baz".
const MC_MARKER_START_RE = /^([A-Za-z])[)\:\.]\s/;
// Same marker anywhere in a string, anchored to start or whitespace so we don't
// match letters inside words ("v.a." or "B.B.King" stays out).
const MC_MARKER_RE = /(?:^|\s)([A-Za-z])[)\:\.]\s/g;

/** Returns the answer's MC letter (lowercased) if it looks like "A) ...", else null. */
function mcAnswerLetter(answer) {
  const m = answer.match(MC_MARKER_START_RE);
  return m ? m[1].toLowerCase() : null;
}

/** Returns the set of MC letters (lowercased) found in a text. */
function mcLettersIn(text) {
  const set = new Set();
  if (!text) return set;
  let m;
  MC_MARKER_RE.lastIndex = 0;
  while ((m = MC_MARKER_RE.exec(text)) !== null) {
...
/**
 * True when `answer` starts with an MC marker AND the question lists a
 * sequential run of options starting from "a" that includes the answer's
 * letter (e.g. a,b — a,b,c — a,b,c,d). An isolated "Z)" elsewhere in the
 * question does not count as an MC list.
 */
function isMultipleChoice(answer, questionDe, questionEn) {
  const ansLetter = mcAnswerLetter(answer);
  if (!ansLetter) return false;
  const letters = new Set([...mcLettersIn(questionDe), ...mcLettersIn(questionEn), ansLetter]);
...
export function validateQuiz({ descriptors, questions, images, quiz, jackpotSize, email, slideOverrides = {} }) {
  ...
  for (let i = 0; i < descriptors.length; i++) {
    ...
    if (desc.type === "question") {
      ...
      // Skip multiple-choice answers whose text is listed as one option among others in the question.
      if (!desc.withAnswers && hasText) {
        const leaked = [];
        for (const ans of [ansDe, ansEn]) {
          if (!ans || ans.length < 3 || leaked.includes(ans)) continue;
          if (isMultipleChoice(ans, de, en)) continue;
    ...
    if (desc.type === "title") {
      // Skip media check for: round-title answer slide (title-r{n}-ans, mirrors the
      // question-phase title) and Antworten dividers (antworten-s{n}, "swap papers"
      // interludes that typically don't carry media).
      const skipMediaCheck = desc.id?.endsWith("-ans") || /^antworten-s\d+$/.test(desc.id || "");
      const slideKey = `${desc.id}:0`;
      if (!skipMediaCheck && !hasMedia(images, slideKey)) {
        issues.push({
          severity: "info",
          descIdx: i,
    ...
    if (desc.type === "intro") {
      ...
      // unless the template opts out via noImages.
      const supportsImages = desc.introIndex == null ? true : desc.introIndex >= 3;
      if (!supportsImages || desc.data?.noImages) continue;
      const slideKey = `${desc.id}:0`;
