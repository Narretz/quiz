 ...
 * @param {Set<string>} [opts.videoSlideNumbers] - slide numbers (as strings) that contain video.
 *   These get click-to-play timing XML added (same mediacall/playFrom preset as audio) so clicking or
 *   pressing space plays the video instead of advancing the slide.
 * @param {Record<string,string>} [opts.videoDurations] - slideNumber (string) → duration ms (string)
 * @param {Set<number>} [opts.revealSlides] - 1-based slide numbers that should fade-in the answer on click
 */
export async function postProcessPptx(arrayBuffer, { audioDurations = {}, audioSlideNumbers = null, videoSlideNumbers = new Set(), videoDurations = {}, revealSlides = new Set() } = {}) {
  ...
  for (const path of Object.keys(files)) {
    ...
    const hasVideo = videoSlideNumbers.has(String(slideNum));
    const wantReveal = revealSlides.has(slideNum);
    if (!hasAudio && !hasVideo && !wantReveal) continue;
    ...
    if (hasVideo && !hasAudio) {
      const spidMatch = xml.match(/<p:cNvPr id="(\d+)" name="Media/);
      if (spidMatch) {
        steps.push({ type: "audio", spid: spidMatch[1], dur: videoDurations[String(slideNum)] || "1" });
      }
    }
