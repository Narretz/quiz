...
export const INTRO_SLIDES = [
  ...
  // --- Slide 4: Golden Rules ---
  {
    ...
    sections: [
      {
        lineValign: "middle",
        lines: [
          { runs: [{ text: "1. Der Quizmaster hat meistens immer recht ⬧ The quizmaster is mostly always right" }] },
          { runs: [{ text: "2. Keine Handys (Taschenrechner erlaubt) ⬧ No Phones (calculators are fine)" }] },
        ],
      },
    ],
    defaultFontSize: 23,
    ...
    sectionGap: 0,
    lineHeight: 1.2,
    // Compact positioning used when an image is added to the slide. Title stays at titleY.
    // Smaller font + tighter spacing leaves room for the image below.
    compactWhenImage: {
      defaultFontSize: 18.5,
      sectionStartY: 0.85,
      lineHeight: 0.4,
    },
    reveal: "image",
...
export const EXTRA_SLIDES = {
  ...
  "break-1": {
    ...
    style: "begin",
    lines: [
      { text: "Pause ⬧ Break", fontSize: 30, bold: true, color: null },
    ...
    compactWhenImage: { fontSizeScale: 0.9 },
  },
  ...
  "points": {
    ...
    compactWhenImage: { fontSizeScale: 0.8 },
  },
  ...
  "goodbye": {
    ...
    sectionGap: 0.8,
    lineHeight: 0.4,
    // Tighter font + spacing when an image is present so the text fits in the
    // top half and the image gets enough room below.
    compactWhenImage: {
      defaultFontSize: 16,
      sectionStartY: 1.0,
      sectionGap: 0.55,
